...
// Upgrade NOTE: replaced 'mul(UNITY_MATRIX_MVP,*)' with 'UnityObjectToClipPos(*)'
	...
	//vertex setup
		...
		useY = lerp(0.0,useY,_heightProjection);
			...
			//normalize
			//useY = lerp(useY,0.0,(shoreTex.r + shoreTex.b) * _shorelineNorm * 0.5);
		...
		//normalize`
		//useY = lerp(useY, 0.0, (1-saturate(shoreTex.r + shoreTex.b)) * _shorelineNorm);
	...
	half4 LightingSuimonoLight (SurfaceOutput s, half3 lightDir, half3 viewDir, half atten){
		...
		//sharpen reflection
		fresnel = saturate(lerp(_reflecSharp,1,fresnel));
	...
	half4 LightingSuimonoLightUnder (SurfaceOutput s, half3 lightDir, half3 viewDir, half atten){
		...
		col.rgb  = lerp(col.rgb, useReflectColor.rgb * 0.75, saturate(lerp(-2.0,6.0,dot(-viewDir*half3(1,2.2,1),half3(0,1,0)))) * _isPlaying ) * _LightColor0.rgb * _LightColor0.a;
		...
		if (suimono_trans_on == 1.0){
			...
			col.rgb = lerp(col.rgb, screenTransparentDistort.rgb, underREFL3 * _isPlaying);
...
//SURFACE --------------------------------------------------------------------------------------------------------------
    void SuimonoSurf (Input IN, inout SurfaceOutput o) {
		...
		float sceneZ2 = 0;
			...
			_distortMask = saturate(saturate(lerp(-0.1,1.0,100.0 * depthDifferential))*20.0);
